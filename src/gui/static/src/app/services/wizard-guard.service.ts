...
export class WizardGuardService implements CanActivate {
  ...
  canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean> {
    return new Promise<boolean>(resolve => {
      // Check if the wallets have been loaded.
      this.walletsAndAddressesService.walletsAlreadyLoaded.pipe(filter(val => val), first()).subscribe(() => {
        // Go to the wizard if there are no wallets.
