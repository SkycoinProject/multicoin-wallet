...
export class LanguageService {
  ...
  private savedSelectedLanguageLoadedInternal = new ReplaySubject<boolean>(1);
  ...
  private loadCurrentLanguage() {
    this.storageService.get(StorageType.CLIENT, this.storageKey).subscribe(response => {
      if (response && this.translate.getLangs().indexOf(response) !== -1) {
        setTimeout(() => { this.changeLanguage(response); }, 16);
