import { throwError as observableThrowError, of, Observable, Subject, throwError } from 'rxjs';
...
export class HwWalletService {
  ...
  /**
   * Checks if all the addresses on an array are valid. Uses concurrency.
  ...
  private verifyAddresses(addresses: string[], currentIndex: number): Observable<any> {
    return this.walletUtilsService.verifyAddress(addresses[currentIndex]).pipe(mergeMap(valid => {
      if (valid) {
