...
export class CreateWalletFormComponent implements OnInit, OnDestroy {
  ...
  validateForm() {
    ...
    // Check if the currently selected coin is compatible with software wallets.
    ...
    // validate the xPub field.
    if (this.activateXPubField) {
      if (!this.form.get('xpub').value) {
        valid = false;
        if (this.form.get('xpub').touched) {
          this.xPubErrorMsg = 'wallet.new.xpub-error-info';
        }
      }
    }
