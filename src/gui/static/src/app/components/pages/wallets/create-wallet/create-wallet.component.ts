...
export class CreateWalletComponent implements OnDestroy {
  ...
  checkAndCreateWallet() {
    ...
    if (this.data.create && this.formControl.getData().type !== WalletTypes.XPub) {
    ...
    } else if (this.data.create || this.synchronized) {
      this.continueCreating();
    } else {
      const confirmationParams: ConfirmationParams = {
  ...
  private continueCreating() {
    ...
    this.walletsAndAddressesService.createWallet(args)
      .subscribe(() => {
        this.busy = false;
