...
export class SignRawTxComponent extends OfflineDialogsBaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    // Get the wallet list.
    this.walletsSubscription = this.walletsAndAddressesService.currentWallets.pipe(first()).subscribe(wallets => {
      if (wallets) {
        this.dropdownElements = [];

        // Create a list with the software wallets, for the dropdown control.
        wallets.forEach(wallet => {
          if (!wallet.isHardware && wallet.walletType !== WalletTypes.XPub) {
