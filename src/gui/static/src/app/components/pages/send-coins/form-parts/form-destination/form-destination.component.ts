...
export class FormDestinationComponent implements OnInit, OnDestroy {
  ...
  private askIfConvertAmount() {
    ...
    const currentCoinName = this.coinService.currentCoinInmediate.coinSymbol;
    let fromText: string;
    let toText: string;
    if (this.selectedCurrency === DoubleButtonActive.LeftButton) {
      fromText = usd;
      toText = currentCoinName;
    } else {
      fromText = currentCoinName;
      toText = usd;
    }
  ...
  private convertAmounts() {
    ...
    this.destControls.forEach(dest => {
      ...
      if (value) {
        ...
        if (this.selectedCurrency === DoubleButtonActive.LeftButton) {
          const newValue = currentValue.dividedBy(this.price).decimalPlaces(this.nodeService.currentMaxDecimals);
          ...
          const recoveredValue = newValue.dividedBy(this.price).decimalPlaces(this.nodeService.currentMaxDecimals);
          if (!recoveredValue.isEqualTo(currentValue)) {
            valuesWithPrecisionErrors += 1;
          }
  ...
  assignAll(index: number) {
    ...
    if (this.selectedCurrency === DoubleButtonActive.LeftButton) {
      availableBalance = availableBalance.decimalPlaces(this.nodeService.currentMaxDecimals, BigNumber.ROUND_FLOOR);
    } else {
    ...
    this.destControls.forEach((dest, i) => {
      ...
      if (!value) {
        ...
        if (!inputInUsd) {
          ...
          const convertedValue = value.dividedBy(currentPrice).decimalPlaces(this.nodeService.currentMaxDecimals);
  ...
  removeDestination(index) {
    ...
    // Remove the associated entries in the error arrays, if needed.
  ...
  validateForm() {
    ...
    if (destinationsHours.isGreaterThan(this.availableBalance.availableHours) && this.showHourFields) {
  ...
  private getAmount(stringValue: string, checkingCoins: boolean): BigNumber {
    ...
    if (checkingCoins) {
      ...
      if (this.selectedCurrency === DoubleButtonActive.LeftButton) {
        if (parts.length === 2 && parts[1].length > this.nodeService.currentMaxDecimals) {
          return null;
        }
      } else {
