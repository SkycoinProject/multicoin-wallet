...
export class SendVerifyComponent implements OnDestroy {
  ...
  send() {
    ...
    // Transactions made with bip44 wallets are already signed.
    if (this.transaction.wallet.encrypted && !this.transaction.wallet.isHardware && this.transaction.wallet.walletType !== WalletTypes.Bip44) {
  ...
  private finishSending(passwordDialog?: any) {
    ...
    // Transactions made with bip44 wallets are already signed. Transactions made with other
    // wallet types must be signed.
    let operation: Observable<string>;
    if (this.transaction.wallet.walletType === WalletTypes.Bip44) {
      operation = of(this.transaction.encoded);
    } else {
      operation = this.spendingService.signTransaction(
    ...
    this.sendSubscription = operation.pipe(mergeMap(encodedSignedTx => {
