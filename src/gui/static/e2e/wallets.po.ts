...
export class WalletsPage {
  ...
  fillWalletForm(label: string, seed: string, confirm: string|null, goToManualSeedMode = true) {
    ...
    return element.all(by.css('[formcontrolname="type"]')).then(test => {

      if (test.length === 0) {
        element(by.css('app-arrow-link')).click();
      }

      const type = element(by.css('[formcontrolname="type"]'));
      ...
      return element.all(by.tagName('option')).then(options => {
        options[0].click();
        ...
        return btn.isEnabled().then(status => {
          ...
          return this.getSafeguardIsShown().then(safeguardIsShown => {
            if (safeguardIsShown) {
              return this.acceptSafeguard().then(() => {
                return status;
              });
            } else {
              return status;
  ...
  addAddress() {
    return element.all(by.css('.-record')).count().then(originalCount => {
      return element(by.css('.-options')).click().then(() => {
        return browser.sleep(2000).then(() => {
          return element(by.css('.e2e-add')).click().then(() => {
            return browser.sleep(2000).then(() => {
  ...
  showChangeWalletName() {
    return element(by.css('.-options')).click().then(() => {
      return element(by.css('.e2e-rename')).click().then(() => {
      ...
      return label.sendKeys('New Wallet Name').then(() => {
        return btn.click().then(() => {
          return browser.sleep(1000).then(() => {
            return element(by.css('body')).sendKeys(protractor.Key.ESCAPE).then(() => {
  ...
  canEncrypt() {
    return element(by.css('.-options')).click().then(() => {
      return element(by.css('.e2e-encrypt')).click().then(() => {
        ...
        return btn.click().then(() => {
          return browser.wait(
            ...
            return element(by.css('.e2e-decrypt')).isPresent().then(response => {
              return element(by.css('body')).sendKeys(protractor.Key.ESCAPE).then(() => {
                return response;
              });
            });
  ...
  canDecrypt() {
    return element(by.css('.-options')).click().then(() => {
      return element(by.css('.e2e-decrypt')).click().then(() => {
        const p1 = element(by.css('[formcontrolname="password"]'));
        const btn = element(by.buttonText('Proceed'));
        ...
        return btn.click().then(() => {
          return browser.wait(
            ...
            return element(by.css('.e2e-encrypt')).isPresent().then(response => {
              return element(by.css('body')).sendKeys(protractor.Key.ESCAPE).then(() => {
                return response;
              });
            });
  ...
  acceptSafeguard() {
    return element.all(by.css('.e2e-confirm-checkbox')).first().click().then(() => {
  ...
  private getWalletWithName(name: string) {
    return element.all(by.css('.e2e-wallets.ng-star-inserted'))
      .filter(wallet => wallet.element(by.css('.-label .text-truncate')).getText().then(text => text === name))
      .first();
  }
